...
Attestation-Based Client Authentication {{ATTEST}} allows a Client
...
trust to deployments ({{ATTEST, Section 10.8}}). It does not define a
...
Client Attester to speak for that client. It does not make that
attester trusted by the AS. The AS remains responsible for deciding
whether to accept the endorsed attester and how its verification keys
are trusted ({{trust}}).

This separates two authorities that otherwise tend to be conflated:
...
({{metadata}}). It applies to both registered clients and clients
identified by CIMDs, and supports several simultaneous endorsements for
cases such as heterogeneous platforms and attester migration.
...
Deployments that can manage client-to-attester relationships entirely
through AS configuration can continue to use {{ATTEST}} without this
profile. This profile is intended for deployments where that
association needs to be expressed and maintained by the client
publisher while remaining subject to AS acceptance policy.
...
client authentication or as an additional security signal. In a typical
deployment those are the token endpoint, the pushed authorization
...
clients and is out of scope. A party authenticating at any of these is
acting as a client, including a resource server presenting a Client
Attestation to the introspection endpoint. Where a flow authenticates
more than once, each presentation is evaluated on its own under
{{as-processing}}. The profile retains ATTEST's wire format, proof
methods, and token binding.
...
and it has a consequence: withdrawing an endorsement ({{updates}}) does
not reach a resource server validating attestations directly. This is
distinct from a resource server authenticating to an AS endpoint, which
acts as a client and is in scope above.
...
attested key. Where DPoP is used alongside a separate Client
Attestation proof, that same section does not require the DPoP key to
match the attestation's `cnf`, and the token is bound to the DPoP key
instead.
